...
Proof.
  ...
  - suffices: exists c2 c1, c2 ∈ γ[itv] (l2, h2)
    ...
    case Hs2: (ZInterval.is_singleton (l2, h2)) => [x2|] Hmb.
    + (* γ2 = {x2}; pick c1 ∈ γ1 with c1 ≠ x2 *)
      have Hns1 : ZInterval.is_singleton (l1, h1) <> Some x2.
      { move: Hmb. case: (ZInterval.is_singleton (l1, h1)) => [x1|//].
    ...
    + (* γ2 has multiple elements; any c1 works. *)
      ...
      have Hns2 : ZInterval.is_singleton (l2, h2) <> Some c1 by rewrite Hs2.
  ...
  - suffices H: not (exists c2 c1, c2 ∈ γ[itv] (l2, h2)
    ...
    destruct (ZInterval.is_singleton (l1, h1)) as [x1|] eqn:Hs1 => //.
    destruct (ZInterval.is_singleton (l2, h2)) as [x2|] eqn:Hs2 => //.
...
Proof.
  ...
  move: Hs1 Hs2. rewrite /ZInterval.is_singleton.
